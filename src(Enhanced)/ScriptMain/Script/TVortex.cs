    ...
    public class TornadoVortex : ScriptExtension
    {
        ...
        public void ChangeDestination(bool trackToPlayer)
        {
            SafeRun(() =>
            {
                for (int i = 0; i < 50; i++)
                {
                    _destination = trackToPlayer
                        ? _player.Position.Around(130.0f)
                        : Helpers.GetRandomPositionFromCoords(_destination, 100.0f);

                    _destination.Z = World.GetGroundHeight(_destination) - 10.0f;
                    var nearestRoadPos = World.GetNextPositionOnStreet(_destination);

                    if (_destination.DistanceTo(nearestRoadPos) < 40.0f &&
                        Math.Abs(nearestRoadPos.Z - _destination.Z) < 10.0f)
                        break;
                }
            }, "ChangeDestination");
        }
            ...
            SafeRun(() =>
            {
                ...
                for (var layerIdx = 0; layerIdx < maxLayers; layerIdx++)
                {
                    for (var angle = 0; angle < (layerIdx > maxLayers - 4 ? particleCount + 5 : particleCount); angle++)
                    {
                        ...
                        {
                            CrashLogger.LogError(ex, $"TVortex Build() small particle spawn failed at layer {layerIdx}");
                        }

                        // Cloud/top layer particles
                        try
                        {
                            ...
                            particle = new TornadoParticle(this, position, rotation, particleAsset, particleName, radius,
                                layerIdx, bIsTopParticle);
                        ...
                        {
                            CrashLogger.LogError(ex, $"TVortex Build() main particle spawn failed at layer {layerIdx}");
                        }
                    }
                }
            }, "Build");
        ...
        private void CollectNearbyEntities(int gameTime, float maxDistanceDelta)
        {
            ...
            SafeRun(() =>
            {
                ...
                foreach (var ent in all)
                {
                    try
                    {
                        if (ent == null || !ent.Exists() || IsInvalidVector(ent.Position)) continue;
                        ...
                        if (ent is Ped ped && !ped.IsRagdoll && ped.Exists())
                            SafeNativeCall(() => Function.Call(Hash.SET_PED_TO_RAGDOLL, ped.Handle, 800, 1500, 2, 1, 1, 0));

                        AddEntity(new ActiveEntity(ent, 3.0f * Probability.GetScalar(), 3.0f * Probability.GetScalar()));
                    }
                    catch (Exception exEnt)
                    {
                        CrashLogger.LogError(exEnt, "CollectNearbyEntities: entity loop failed");
                    }
                }
            ...
            SafeRun(() =>
            {
                ...
                foreach (var kvp in snapshot)
                {
                    try
                    {
                        ...
                        if (entity == null || !entity.Exists() || IsInvalidVector(entity.Position))
                        {
                            ReleaseEntity(key);
                            continue;
                        }
                        ...
                        if (dirVec.Length() < 0.0001f) continue;
                        ...
                        if (value.IsPlayer)
                        {
                            float verticalForce = globalVerticalForce * 1.62f;
                            float horizontalForce = globalHorizontalForce * 1.2f;
                            ...
                            if (gameTime - _lastPlayerShapeTestTime > 1000)
                            {
                                ...
                                catch { _lastRaycastResultFailed = true; }
                            ...
                            SafeNativeCall(() => Function.Call(Hash.SET_ENTITY_MAX_SPEED, entity.Handle, topSpeed));
                        }
                        else
                        {
                            float verticalForceNonPlayer = globalVerticalForce;
                            float horizontalForceNonPlayer = globalHorizontalForce;

                            var model = entity.Model;
                            if (model != null && model.IsValid && model.IsPlane)
                            {
                                force *= 6.0f;
                                verticalForceNonPlayer *= 6.0f;
                            }

                            entity.ApplyForce(direction * horizontalForceNonPlayer, new Vector3(Probability.NextFloat(), 0, Probability.GetScalar()));
                            var upDir2 = Vector3.Normalize(new Vector3(_position.X, _position.Y, _position.Z + 1000.0f) - entity.Position);
                            entity.ApplyForceToCenterOfMass(upDir2 * verticalForceNonPlayer);
                            var cross2 = Vector3.Cross(direction, Vector3.WorldUp);
                            entity.ApplyForceToCenterOfMass(Vector3.Normalize(cross2) * force * horizontalForceNonPlayer);
                    ...
                    {
                        CrashLogger.LogError(exNative, $"UpdatePulledEntities caught entity {kvp.Key}");
        ...
        public override void OnUpdate(int gameTime)
        {
            SafeRun(() =>
            {
                ...
                if (ScriptThread.GetVar<bool>("vortexMovementEnabled"))
                {
                    ...
                    var vTarget = MathEx.MoveTowards(_position, _destination,
        ...
        public override void Dispose()
        {
            SafeRun(() =>
            {
                ...
                {
                    try { particle.Dispose(); }
                    catch (Exception ex) { CrashLogger.LogError(ex, "Dispose: particle disposal failed"); }
                }
        ...
        private static void SafeNativeCall(Action nativeAction)
        {
            try { nativeAction(); }
            catch { /* silently ignore native crashes */ }
        }
