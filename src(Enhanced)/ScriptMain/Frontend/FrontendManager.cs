...
namespace TornadoScript.ScriptMain.Frontend
{
    public class FrontendManager : ScriptExtension
    {
        ...
        public override void OnUpdate(int gameTime)
        {
            ...
            if (_showingConsole)
            {
                if (Game.IsControlJustPressed((GTA.Control)241) ||
                    Game.IsControlJustPressed((GTA.Control)188))
                {
                    _output.ScrollUp();
                }

                else if (Game.IsControlJustPressed((GTA.Control)242) ||
                         Game.IsControlJustPressed((GTA.Control)187))
                {
                    _output.ScrollDown();
                }
            }
